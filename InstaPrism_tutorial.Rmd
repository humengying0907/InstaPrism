...
* **cell.types.to.update**: a user-defined vector indicating which (non-malignant) cell.types to update. In BayesPrism, all the cell.types are automatically updated with the pooled bulk information (the Z array), irrespective of its rationality. We now provide this `cell.types.to.update` argument such that users can decide which cell.types to update the reference for. 
...
To update the reference profile for the above simulated dataset, we will set `key = NA` since no malignant cells are present in this problem. Note that when we set `cell.types.to.update` equals to all the unique cell.types present in the reference profile, the deconvolution results obtained from `InstaPrism_update()` will be nearly identical to the BayesPrism updated results. This is because `InstaPrism_update()` pooled the same bulk expression information for reference update as BayesPrism.
...
updated_obj = InstaPrism_update(InstaPrism.res.initial, 
                                ...
                                n.iter = 100, 
                                n.core = 16, 
                                # choose all non-malignant cell.types to update for full comparison with BayesPrism
                                cell.types.to.update = rownames(InstaPrism.res.initial@Post.ini.ct@theta)[2:6], 
                                key = 'tumor')

# 4. merge cell.state level theta to cell.type level 
updated_ct = merge_updated_theta(updated_obj,key = 'tumor')

# note: since no cell.state associated information present in this problem, the following two matrices are the same
# all.equal(updated_ct,updated_obj@theta) # TRUE

# 5. reconstruct cell.type specific expression with the updated reference (optional)
Z_tumor_ct_updated = reconstruct_Z_ct_updated(updated_obj,cell.type.of.interest = 'tumor',key = 'tumor',n.core = 16)
```
...
```{r}
ct.corr.updated=cor(t(updated_ct),bp.res@posterior.theta_f@theta)
...
InstaPrism_running_time = as.numeric(elp$toc - elp$tic) / 60 # tic and toc are the elapsed process times in seconds, divid it by 60 to convert to minutes
...
##### 5. reconstruct cell.type specific expression with the updated reference (optional)
```{r,eval=FALSE}
Z_tumor_ct_updated = reconstruct_Z_ct_updated(updated_obj,cell.type.of.interest,key = 'tumor')
```
...
* **n.core**: number of cores to use for parallel programming. We recommend setting this parameter to higher value when multiple cores are available
